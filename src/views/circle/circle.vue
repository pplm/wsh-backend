        ...
        <Row>
            ...
            <Col span="6" style="margin-bottom: -15px;">
                <FormItem label="是否有代理商" prop="agent_flag">
                    <Select v-model="queryForm.agent_flag" clearable placeholder="请选择agent_flag" style="width:174px">
                        <Option v-for="item in dict.agent_flag" :value="item.value" :key="item.value">{{ item.label }}</Option>
                    </Select>
                </FormItem>
            </Col>
            <Col span="6" style="margin-bottom: -15px;">
...
export default {
    ...
    methods: {
        ...
        doSave () {
            let _self = this;
            util.ajax.post('/CircleManagement/CircleList/save', this.saveForm).then(res => {
        ...
        doDelete (id) {
            ...
            this.$Modal.confirm({
                ...
                onOk: () => {
                    ...
                    util.ajax.post('/CircleManagement/CircleList/delete/' + id).then(res => {
                        _modal.remove()
                        if (res.status === 200) {
                            if (res.data.code === "0") {
                                _self.$Message.info('操作成功')
        ...
        doExport () {
            let _self = this
            util.ajax.post('/CircleManagement/CircleList/Export', this.processQueryForm()).then(res => {
                if (res.status === 200) {
        ...
        getList () {
            let _self = this
            util.ajax.get('/CircleManagement/CircleList?page=' + this.page.current + '&limit=' + this.page.size, {params: this.processQueryForm()}).then(res => {
                if (res.status === 200) {
                    if (res.data.result === 1) {
        ...
        showModalAdd() {
            this.clearSaveForm()
            this.saveModal.title = '添加C01F03A1圈子管理列表'
            this.saveModal.show = true
        },
        showModalUpdate(id) {
            let _self = this
            util.ajax.get('/CircleManagement/CircleList/detail?id=' + id).then(res => {
                if (res.status === 200) {
                    if (res.data.code === "0") {
            ...
            this.saveModal.show = true
            this.saveModal.title = '修改C01F03A1圈子管理列表'
        },
        clearSaveForm () {
            this.saveForm.id = ''
            this.saveForm.name = ''
